...
//Edited By Sir.D.Kai Studio

#include "Animation/AnimNode_KLSOrientationWarping.h"
#include "Animation/AnimInstanceProxy.h"
#include "Animation/AnimRootMotionProvider.h"

DECLARE_CYCLE_STAT(TEXT("KLS OrientationWarping Eval"), STAT_OrientationWarping_Eval, STATGROUP_Anim);

#if ENABLE_ANIM_DEBUG
static TAutoConsoleVariable<int32> CVarAnimNodeOrientationWarpingDebug(TEXT("a.AnimNode.KLSOrientationWarping.Debug"), 0, TEXT("Turn on visualization debugging for Orientation Warping. 0 == off, 1 == Warping, 2 == Offset, 3 == Both."));
static TAutoConsoleVariable<int32> CVarAnimNodeOrientationWarpingVerbose(TEXT("a.AnimNode.KLSOrientationWarping.Verbose"), 0, TEXT("Turn on verbose graph debugging for Orientation Warping"));
static TAutoConsoleVariable<int32> CVarAnimNodeOrientationWarpingEnable(TEXT("a.AnimNode.KLSOrientationWarping.Enable"), 1, TEXT("Toggle Orientation Warping"));
#endif
...
void FAnimNode_KLSOrientationWarping::GatherDebugData(FNodeDebugData& DebugData)
{
...
void FAnimNode_KLSOrientationWarping::Initialize_AnyThread(const FAnimationInitializeContext& Context)
{
...
void FAnimNode_KLSOrientationWarping::UpdateInternal(const FAnimationUpdateContext& Context)
{
...
void FAnimNode_KLSOrientationWarping::EvaluateSkeletalControl_AnyThread(FComponentSpacePoseContext& Output, TArray<FBoneTransform>& OutBoneTransforms)
{
...
bool FAnimNode_KLSOrientationWarping::IsValidToEvaluate(const USkeleton* Skeleton, const FBoneContainer& RequiredBones)
{
#if ENABLE_ANIM_DEBUG
...
void FAnimNode_KLSOrientationWarping::InitializeBoneReferences(const FBoneContainer& RequiredBones)
{
...
void FAnimNode_KLSOrientationWarping::CacheBones_AnyThread(const FAnimationCacheBonesContext& Context)
{
	GetEvaluateGraphExposedInputs().Execute(Context);
	SpineBones.Reset();
	for(FName SpineBone : SpineBonesNames)
	{
		FBoneReference BoneRef = FBoneReference(SpineBone);
		BoneRef.BoneIndex = INDEX_NONE;
		SpineBones.Add(BoneRef);
	}
	IKFootBones.Reset();
	for(FName IKFootBone : IKFootBonesNames)
	{
		FBoneReference BoneRef = FBoneReference(IKFootBone);
		BoneRef.BoneIndex = INDEX_NONE;
		IKFootBones.Add(BoneRef);
	}
...
void FAnimNode_KLSOrientationWarping::Reset(const FAnimationBaseContext& Context)
{
	bIsFirstUpdate = true;
	HeadingOffset = 0.0f;
