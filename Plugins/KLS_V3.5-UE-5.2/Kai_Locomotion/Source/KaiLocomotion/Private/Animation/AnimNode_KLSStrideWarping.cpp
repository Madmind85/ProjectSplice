...
#include "Animation/AnimNode_KLSStrideWarping.h"
...
void FAnimNode_KLSStrideWarping::GatherDebugData(FNodeDebugData& DebugData)
{
...
void FAnimNode_KLSStrideWarping::Initialize_AnyThread(const FAnimationInitializeContext& Context)
{
...
void FAnimNode_KLSStrideWarping::UpdateInternal(const FAnimationUpdateContext& Context)
{
	FAnimNode_SkeletalControlBase::UpdateInternal(Context);
	CachedDeltaTime = Context.GetDeltaTime();
}

void FAnimNode_KLSStrideWarping::EvaluateSkeletalControl_AnyThread(FComponentSpacePoseContext& Output, TArray<FBoneTransform>& OutBoneTransforms)
{
...
bool FAnimNode_KLSStrideWarping::IsValidToEvaluate(const USkeleton* Skeleton, const FBoneContainer& RequiredBones)
{
...
void FAnimNode_KLSStrideWarping::UpdateBoneReferencesFromNames()
{
	//Set The Bone references From The Names.
	FootDefinitions.Reset();
	for(FStrideWarpingFootDefinitionName FootDef : FootDefinitionsNames)
	{
		FStrideWarpingFootDefinition FinalFootDef;
		FinalFootDef.ThighBone = FBoneReference(FootDef.ThighBone);
		FinalFootDef.ThighBone.BoneIndex = INDEX_NONE;
		FinalFootDef.IKFootBone = FBoneReference(FootDef.IKFootBone);
		FinalFootDef.IKFootBone.BoneIndex = INDEX_NONE;
		FinalFootDef.FKFootBone = FBoneReference(FootDef.FKFootBone);
		FinalFootDef.FKFootBone.BoneIndex = INDEX_NONE;
		FootDefinitions.Add(FinalFootDef);
	}
...
void FAnimNode_KLSStrideWarping::CacheBones_AnyThread(const FAnimationCacheBonesContext& Context)
{
	GetEvaluateGraphExposedInputs().Execute(Context);
	UpdateBoneReferencesFromNames();
